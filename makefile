...
main.bin: main.c linker.lds
	gcc -c -m32 -nostdlib -nodefaultlibs -fno-exceptions -static main.c -fno-pie -fno-builtin
	ld -melf_i386 -o main.bin -T linker.lds main.o

bootdisk.img: bootloader.bin main.bin
	dd if=/dev/zero of=bootdisk.img bs=512 count=3
	dd conv=notrunc if=bootloader.bin of=bootdisk.img bs=512 seek=0 count=1
	dd conv=notrunc if=main.bin of=bootdisk.img bs=512 seek=1 count=2
